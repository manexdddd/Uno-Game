...
def cargar_imagenes_cartas():
    ...
    colores = ['Rojo', 'Verde', 'Azul', 'Amarillo']
    valores = list(range(0, 10))
    cartas_especiales = ['Skip', 'Reverse','+4']
    ancho_deseado = 70
    alto_deseado = 100
...
def crear_mazo():
    colores = ['Rojo', 'Verde', 'Azul', 'Amarillo']
    valores = list(range(0, 10)) + ['Skip', 'Reverse','+4']
...
def aplicar_efecto_carta(carta, mazo, oponente_cartas):
    ...
    if carta.valor == '+4':
        # El jugador oponente toma 4 cartas y pierde su turno
        for _ in range(4):
            carta_nueva = tomar_carta(mazo, oponente_cartas)
            if carta_nueva:
                print(f"Se ha tomado una carta: {carta_nueva}")
        return False  # El turno pasa al oponente
...
def main():
    ...
    while corriendo:
        ...
        if len(cartas_jugador) >= 12:
            start_pc()
            corriendo = False
            break
        elif len(cartas_computadora) >= 12:
            start_jugador()
            corriendo = False
            break
