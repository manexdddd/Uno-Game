...
pygame.display.set_caption("UNO - Ganador Computadora")
pygame.mixer.music.load("music/snow.mp3")
...
background = pygame.image.load('images/fondos/sas.jpg').convert()
...
win = pygame.image.load('images/fondos/pc.png').convert_alpha()
...
wins= pygame.transform.scale(win, (1024/2, 182/2))
...
def main():
    ...
    while True:
       ...
       # Dibujar título (ahora es el logo escalado)
        logo_rect = scaled_logo.get_rect(center=(screen_width // 2, 140))
        screen.blit(scaled_logo, logo_rect)
        uno_rect = scaled_uno.get_rect(center=(300, 470))
        screen.blit(scaled_uno, uno_rect)

        dos_rect = scaled_dos.get_rect(center=(720, 470))
